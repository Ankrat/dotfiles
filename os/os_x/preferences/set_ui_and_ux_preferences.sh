...
set_preferences() {
    ...
    execute "sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.smb.server NetBIOSName -string 'BW-MacBook-Yann' \
                && sudo scutil --set ComputerName 'BW-MacBook-Yann' \
                && sudo scutil --set HostName 'BW-MacBook-Yann' \
                && sudo scutil --set LocalHostName 'BW-MacBook-Yann'" \
        "Set computer name"
    ...
    execute "   for domain in ~/Library/Preferences/ByHost/com.apple.systemuiserver.*; do
            ...
            " \
        "Hide Time Machine, AirPort and Volume icons from the menu bar"
